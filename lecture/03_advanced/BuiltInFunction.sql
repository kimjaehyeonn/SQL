...
-- 제어흐름함수: if, ifnull , CASE ~ WHEN ~ ELSE ~ END
-- 커미션이 NULL인 경우 0으로 대체하여 보여줌, JAVA에서 굳이 NULL 확인 코드 안넣어도 됨. 
SELECT ENAME, SAL, IFNULL(COMM, 0) AS COMM_REPLACED
FROM EMP;

SELECT*FROM EMP; -- NULL 확인 가능 
...
-- 문자열 함수: CONCAT
-- CONCAT 함수:  사원 이름과 직책을 하나의 문자열로 합쳐서 출력
SELECT ENAME, JOB, CONCAT(ENAME, ' is a ', JOB) AS EMP_INFO
FROM EMP;

-- UPEER: 모든 사원 이름을 대문자로 출력
SELECT ENAME, UPPER(ENAME) AS UPPER_NAME
FROM EMP;

-- REPLACE: 직책 이름 중 'MANAGER'를 'MGR'로 바꿔 출력
