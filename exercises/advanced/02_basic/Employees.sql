...
-- 조건1. 재직중이고 부서번호가 d005 인 부서의 관리자
-- 조건2. 관리자의 정보
-- 출력 -> 관리자 정보
...
WITH CURRENT_MANAGER AS (select EMP_NO
                         ...
                         where to_date = '9999-01-01'
                           and dept_no = 'd005')
SELECT*FROM EMPLOYEES WHERE EMP_NO = (SELECT EMP_NO FROM CURRENT_MANAGER);

/*employees 테이블에서 페이지네이션으로 페이지를 추출하려고 한다 . 다음 조건하에서 8 번 페이지의 데이터를 출력하세요
○ 입사일을 내림차순으로 정렬한다
○ 한 페이지당 20 명의 정보를 출력한다*/
select*from employees order by hire_date desc limit 140, 20; -- offset, amount

/*employees db 에서 재직자의 총 수를 구하시오
○ 재직자의 to_date 값은 '9999 01 01' 로 저장되어 있음*/
select count(*) from dept_emp where to_date = '9999-01-01';

/*employees db 에서 재직자의 평균 급여를 출력하시오*/
...
-- 서브 쿼리 너무 길어지면, 가독성 떨어지고 유지보수 힘들어짐.

/*employees db 에서 각 부서별 재직자의 수를 구하시오
